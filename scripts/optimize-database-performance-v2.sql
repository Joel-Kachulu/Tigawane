-- Database Performance Optimization Script (Fixed)
...
BEGIN
    ...
    -- Add is_read column as alternative if read doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Collaboration messages indexes (only if table exists)
DO $$ 
BEGIN
...
-- Collaboration participants indexes (only if table exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'collaboration_participants') THEN
...
-- Notifications table indexes (check which column exists)
DO $$ 
BEGIN
    ...
    ELSE
        -- Just create basic indexes if no read status column exists
        CREATE INDEX IF NOT EXISTS idx_notifications_user_id ON notifications(user_id);
        CREATE INDEX IF NOT EXISTS idx_notifications_created_at ON notifications(created_at DESC);
    END IF;
END $$;

-- Admin users indexes (only if table exists)
DO $$ 
BEGIN
...
DROP MATERIALIZED VIEW IF EXISTS item_stats;
...
-- Create function for optimized collaboration message fetching (only if table exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'collaboration_messages') THEN
        EXECUTE '
...
-- Only enable RLS on collaboration_messages if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'collaboration_messages') THEN
